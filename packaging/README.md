...
2. Create a package in your home project and add the spec + release tarball:
   ...
   for a in x86_64 aarch64; do
     curl -fLO https://github.com/0nigiris/JII/releases/download/v0.1.21-beta/jii-v0.1.21-beta-$a-linux.tar.gz
   done
   osc add jii.spec *.tar.gz
   osc commit -m "jii 0.1.21-beta"
   ```
...
3. Commit and push (AUR's branch is `master`):
   ```sh
   git add PKGBUILD .SRCINFO && git commit -m "jii-bin 0.1.21_beta" && git push origin master
   ```
