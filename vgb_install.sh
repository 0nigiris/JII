#!/bin/sh
# JII — the *secret* handheld-Jevil installer (VGB edition).
#
#   curl -fsSL https://raw.githubusercontent.com/0nigiris/JII/chaos/vgb_install.sh | sh
#
# Downloads CherrySodaPop's "Jevil-VGB" — Deltarune's Jevil fight recreated in the style of
# a retro handheld console (Godot, GPL-3.0) — launches it as a real desktop window, and
# installs JII **only once you win**: pacify him to sleep or cut him down, either works.
# The game drops a `chaos-install` marker with the ending; JII reads it on its next run and
# unlocks the secret 🃏 achievement — the same one the Chaos Simulator grants.
#
...
# --- knobs (env overrides) -------------------------------------------------
# The exported game (a single self-contained Godot binary, ~43 MB).
GAME_SRC="${JII_VGB_SRC:-https://github.com/0nigiris/JII/releases/download/chaos-game/jevil-vgb.x86_64}"
...
# Set JII_VGB_NO_INSTALL=1 to play the fight without installing (testing).
NO_INSTALL="${JII_VGB_NO_INSTALL:-0}"
MARKER_NAME="chaos-install"
...
run_normal_install() {
  [ "$NO_INSTALL" = "1" ] && { note "JII_VGB_NO_INSTALL=1 — skipping install."; return 0; }
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/jii-vgb.XXXXXX")"
...
note "A tiny handheld Jevil. Put him to sleep or cut him down — either wins."
...
if [ "$NO_INSTALL" = "1" ]; then
  note "JII_VGB_NO_INSTALL=1 — skipping install (marker left for JII to read)."
  exit 0
fi
